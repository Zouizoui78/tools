...
class Observable {
    ...
    /**
     * @brief Tell whether the passed object has subscribed to the given event or not.
     * @param observer Observer to search for in event's subscribers.
     * @param event_id Event to search observer in.
     * @return subscribed or not. Return false if no event has been registered, if event_id has been registered and if the event_id is invalid.
     */
     ...
     * @param event_id Event ID. If default value ("") is passed, the observer is unsubscribed from every currently existing events.
    ...
    /**
     * @brief Tell the subscribed observers that the given event has been triggered.
     * @param event_id Event ID.
     */
