...
ifeq ($(OS),Windows_NT)
	PATH=$(BINDIR) TEST_OUTPUTS=$(TEST_OUTPUTS) GTEST_OUTPUT="xml:$(TEST_GTEST_OUTPUT_FILE)" GTEST_FILTER=*$(T)* ./$(TEST_OUTPUT)
else
	$(LD_PATH) TEST_OUTPUTS=$(TEST_OUTPUTS) GTEST_OUTPUT="xml:$(TEST_GTEST_OUTPUT_FILE)" GTEST_FILTER=*$(T)* ./$(TEST_OUTPUT)
endif
valgrindtest: $(USEMOD) $(OUTPUT) $(TEST_OUTPUT)
	@echo "Running tests for module $(MODULE_NAME) through valgrind"
	@mkdir -p $(TEST_GTEST_OUTPUT_DIR)
	$(LD_PATH) TEST_OUTPUTS=$(TEST_OUTPUTS) GTEST_OUTPUT="xml:$(TEST_GTEST_OUTPUT_FILE)" GTEST_FILTER=*$(T)* valgrind $(VALGRIND_OPT) $(TEST_OUTPUT)
